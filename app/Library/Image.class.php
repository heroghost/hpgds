...
// +----------------------------------------------------------------------
// | Author: hpduan <hpduan@jingqubao.com>
// +----------------------------------------------------------------------
// $Id$
...
//        @imagestring($im, 5, 5, 3, $randval, $stringColor);
     ...
     * @return string
     +----------------------------------------------------------
     */
    static function generImageVerify($length=4,$mode=1,$type='png',$width=48,$height=22,$verifyName='verify')
    {
    	...
    	//        @imagestring($im, 5, 5, 3, $randval, $stringColor);
    	$verifyImg = TMP_IMG_PATH . '/' . $verifyName.'.png';
        $returnImg = SITE_URL. '/tmpimg/' . $verifyName.'.png';
    	imagebmp($im,$verifyImg);
    	$data["img"] = $returnImg;
    	$data[$verifyName] = md5(strtoupper($randval));
    	return $data;
    }
    
    // 中文验证码
    static function GenerGBVerify($length=4,$type='png',$width=120,$height=30,$fontface='simhei.ttf',$verifyName='verify') {
    	//tsload(ADDON_PATH.'/liberary/String.class.php');
    	$code = String::rand_string($length,4);
    	$width = ($length*45)>$width?$length*45:$width;
    	//$_SESSION[$verifyName]= md5($code);
    	...
    	//Image::output($im,$type);
    	$verifyImg = TMP_IMG_PATH . '/' . $verifyName.'.bmp';
    	imagebmp($im,$verifyImg);
    	$data["img"] = $verifyImg;
    	$data[$verifyName] = md5($code);
    	return $data;
    }
    ...
    static function merge($imgs) {
//        $imgs = array(); 
//        $imgs[0] = '1.jpg'; 
//        $imgs[1] = '2.jpg'; 
//        $imgs[2] = '3.jpg'; 
//        $imgs[3] = '4.jpg'; 
        $target = '/tmp/emp.jpg';//背景图片 
        $target_img = Imagecreatefromjpeg($target); 
        $source= array(); 
        foreach ($imgs as $k=>$v){ 
            $source[$k]['source'] = Imagecreatefromjpeg($v); //var_dump(3);exit;
        ...
        for ($i=0; $i<count($source); $i++){  
            imagecopyresized ($target_img, $source[$i]['source'], $tmp, $tmpy, 0, 0, $tWidth, $tHeight, $source[$i]['size'][0], $source[$i]['size'][1]);
